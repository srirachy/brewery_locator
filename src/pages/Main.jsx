...
const Main = () => {
  ...
  const [location, setLocation] = useState(
    JSON.parse(localStorage.getItem("location")) || "Fresno"
  );
  ...
  const search = (apiData) => {
    if (newSearch === "") {
      ...
      return apiData.filter((value) => {
        if (
          value.name?.toLowerCase().includes(newSearch.toLowerCase()) ||
          value.brewery_type?.toLowerCase().includes(newSearch.toLowerCase()) ||
  ...
  const selectBrewery = useCallback(
    (id) => {
